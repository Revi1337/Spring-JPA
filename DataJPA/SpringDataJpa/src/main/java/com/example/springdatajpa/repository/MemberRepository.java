...
//    List<Member> findByAge(int age, Pageable pageable);
    ...
    @Query(value = "select m from Member as m left join fetch m.team t")
    List<Member> findMemberFetchJoin();                                                 // fetch join 으로 연관된 Entity 들을 한방쿼리로 갖고오기 (DataJPA 에서 제공하는 fetch join (1))

    @Override @EntityGraph(attributePaths = {"team"})                                   // 내부적으로 team 을 fetch join 하겠다는 의미임. (기본적으로 left join)
    List<Member> findAll();                                                             // @EntityGraph 를 사용 JPQL 을 사용하지않아도 됨.. (DataJPA 에서 제공하는 fetch join (2))

    @EntityGraph(attributePaths = "team")                                               // Data jpa 에서 제공하는 fetch join 의 기능은
    @Query(value = "select m from Member as m")                                         // JPQL 과 도 합칠 수 있음
    List<Member> findMemberByEntityGraph();

    @EntityGraph(attributePaths = {"team"})                                             // 쿼리메서드에도 결합이 가능함.
    List<Member> findBlaBlaByUsername(@Param("username") String username);

    @EntityGraph("Member.all")                                                          // 순수 JPA 에서 지원하는 스펙인 NamedEntityGraph 로도 fetch join 가능. (Member 클래스에서 @NamedEntityGraph 부분)
    List<Member> findBlaBla2ByUsername(@Param("username") String username);             // 잘사용하지 않음.
